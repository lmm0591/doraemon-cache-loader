...
const defaultCacheDirectory = path.resolve('.doraemon-cache-loader');
const ENV = process.env.NODE_ENV || 'development';

module.exports = function loader(...args) {
  const callback = this.async();
  const { data } = this;
  var dependencies = this.getDependencies ?  this.getDependencies() : []
  dependencies = dependencies.concat(this.loaders.map(l => l.path));
  const contextDependencies = this.getContextDependencies ? this.getContextDependencies() : []
...
module.exports.pitch = function pitch(remainingRequest, prevRequest, dataInput) {
  const loaderOptions = loaderUtils.getOptions(this) || {};
  const defaultOptions = {
    cacheDirectory: defaultCacheDirectory,
    cacheIdentifier: `doraemon-cache-loader:${pkgVersion} ${ENV}`,
  };
